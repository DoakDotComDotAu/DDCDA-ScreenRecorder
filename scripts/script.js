...
async function startCapture() {
    ...
    const wantCam = recCamera.checked;
...
camEl.muted = true;
camEl.style.display = "none"; // keep hidden
...
//overlayImg.src = "../assets/video/overlay.png"; // change to your image file

// --- Position helper ---
function getPosition(pos, w, h, padding = 10) {
    let x = 0, y = 0;
    switch (pos) {
        case "top-left":
            x = padding; y = padding; break;
        case "top-center":
            x = (canvas.width - w) / 2; y = padding; break;
        case "top-right":
            x = canvas.width - w - padding; y = padding; break;
        case "center":
            x = (canvas.width - w) / 2; y = (canvas.height - h) / 2; break;
        case "bottom-left":
            x = padding; y = canvas.height - h - padding; break;
        case "bottom-center":
            x = (canvas.width - w) / 2; y = canvas.height - h - padding; break;
        case "bottom-right":
...
function draw() {
    ...
    // --- Webcam overlay ---
    if (camEl.readyState >= 2 && wantCam) {
        const camW = 300, camH = 250;
        const { x, y } = getPosition("top-right", camW, camH);
        ctx.drawImage(camEl, x, y, camW, camH);
    } else if (!wantCam) {
        // Stop webcam if not wanted
    }

    // --- Image overlay ---
//    if (overlayImg.complete) {
//        const imgW = 54, imgH = 40;
//        const { x, y } = getPosition("top-left", imgW, imgH);
//        ctx.drawImage(overlayImg, x, y, imgW, imgH);
//    }
...
// ---- END WATERMARK + ELEMENT INJECTION ----
